...
Audio._STREAMS = [];
Audio._addStream = function(audio)
{
    var streams = Audio._STREAMS;
    for (var iAudio = 0; iAudio < streams.length; iAudio++)
    {
        if (streams[iAudio] == audio)
        {
            return false;
        }
    }
    Audio._STREAMS.push(audio);
}
Audio._removeStream = function(audio)
{
    var streams = Audio._STREAMS;
    for (var iAudio = 0; iAudio < streams.length; iAudio++)
    {
        if (streams[iAudio] == audio)
        {
            streams.splice(iAudio, 1);
        }
    }
}
Audio.STOP_ALL = function()
{
    var streams = Audio._STREAMS;
    for (var iAudio = 0; iAudio < streams.length; iAudio++)
    {
