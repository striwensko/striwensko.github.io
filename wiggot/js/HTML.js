...
var StateManager = {};
StateManager.setState = function(data, action)
{
    var self = this;
    if (!this.state)
    {
    ...
    if (!this.__stateManager)
    {
        this.__stateManager = {pending: [], onState: function(){
            ...
            while (pipe.length > 0)
            {
                var __state = pipe.shift();
                pipeCopy.push(__state);
                new_data = Object.merge(new_data, __state.data);
            }
            self.state = Object.merge({}, self.state);
            self.state = Object.merge(self.state, new_data);
            self.__stateManager.requestUpdate = false;
            
            new_data.findAction = function(property)
            {
                var actions = [];
                for (var iItem = 0; iItem < pipeCopy.length; iItem++)
                {
                    if (pipeCopy[iItem].data.hasOwnProperty(property) && pipeCopy[iItem].action)
                    {
                        actions.push(pipeCopy[iItem].action);
                    }
                }
                return actions;
            }
    ...
    this.__stateManager.pending.push({data:data, action:action});
...
Browser = {};
Browser.nodeParse = function(node, scope)
{
