...
TEXT_Loader.prototype.abort = function ()
{
	if (this.request)
	{
		this.request.abort();
...
TEXT_Loader.prototype.send = function(data)
{
	this.load(data.url, data.data, data.contentType || null, data.requestMethod || null, data.headers || null);
}
/**
...
*	@param {string} contentType The contentType attach to the request the default value is "application/x-www-form-urlencoded"
*/
TEXT_Loader.prototype.load = function (url, parameters, contentType, requestMethod, headers)
{
		this.url = url;
		this.data = null;
		this.isReady = false;
