...
Walmart = function(){
  ...
  this.holder = Browser.DOM(html, this.UI);
...
Walmart.Catalog = function(){
  this.UI = {};
  var self = this;
  var html = '';
  html += '<div class="catalog">';
  html += '  <div var="list" class="list"></div>';
  html += '  <b class="more-button" var="buttons.more">More</b>';
  html += '  <b var="loader" class="loader">' + SVG.loader + '</b>';
  html += '</div>';
...
Walmart.Catalog.prototype.createItem = function(data){
  ...
  html += '  <div var="title" class="title"></div>';
  html += '  <img var="img"/>';
  html += '  <u var="msrp" class="msrp"></u>';
  html += '  <i var="price" class="price"></i>';
  html += '  <b var="buttons.addToCart">' + SVG.plusCircle + '</b>';
  html += '</div>';

  var node = Browser.DOM(html, UI);
  if (data.msrp){
...
Walmart.Product = function(){
  ...
  html += '          <b var="qty">0</b>';
  html += '          <u var="buttons.plus">' + SVG.plusIcon + '</u>';
  html += '        </span>';
  html += '      </div>';
  html += '    </div>';
  html += '  </div>';
...
Walmart.ShoppingCart = function(){
  var self = this;
  this.UI = {};
  var html = '';
  html += '<div class="shopping-cart">';
  html += '  <h1>';
  html += '    <b var="buttons.close">' + SVG.angleLeft + '</b>';
  html += '    The shopping list<i var="shopping.qty"></i>';
  html += '  </h1>';
  html += '  <div class="list"><div var="list"></div></div>'
  html += '  <div class="bottom"><b var="buttons.buy">Buy <span var="price.total"></span></b></div>'
  html += '</div>';
...
Walmart.ShoppingCart.Item = function(data){
  ...
  this.UI.title.onclick = function(){
    Walmart.Data.setState('Product', {itemId: data.itemId});
    Walmart.Data.setState('mode', {mode: 'product'});
  }
  this.UI.buttons.minus.onclick = function(){
